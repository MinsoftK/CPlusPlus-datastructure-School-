...
void CircularQueueType<T>::ResetList() {
	Cur_pointer2 = m_iFront;
}
template <typename T>
bool CircularQueueType<T>::GetNextItem() {
	if (!IsEmpty()){
		Cur_pointer2++;
	cout << items.[Cur_pointer2] << endl;
	return true;
	}else false
}
template <typename T>
int CircularQueueType<T>::Search(T) {
	items.Get
}
template <typename T>
void CircularQueueType<T>::Delete(T) {

}
template <typename T>
void CircularQueueType<T>::Replace(T) {
