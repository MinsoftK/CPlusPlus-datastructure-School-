...
public :
	...
	*/
	...
	/**
		*	@brief	Heap의  데이터를 업데이트 하는 함수.
		*	@pre	두개의 객체 데이터를 받는다
		*	@post	두 데이터의 위치가 바뀐다.
		*	@param	item	바뀌는 데이터의 객체.
		*	@param	item2	바꿀 데이터의 객체.
		*/
	void UpdateItem(T& item, T& item2, bool& found);
...
// Heap의 데이터를 수정하는 가상함수.
template <typename T>
void CHeapBase<T>::UpdateItem(T&item,T& item2, bool& found)
{
	Update(item,item2, found, 0); // 데이터 검색
}
