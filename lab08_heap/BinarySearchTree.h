...
public:
	...
	*/
	...
	//Update할 노드를 찾고 입력 받은 객체의 정보를 넣어주고 변경되기전의 정보를 객체에 입력 받는다
	// 총2개의 객체 
...
//Tree에서 바꾸고자하는 값을 Update
template<class T>
void BinarySearchTree<T>::UpdateItem(T& GetItem,T& SetItem, bool& found)const
{
	FindUpdate(root,GetItem,SetItem, found);		// Tree 검색 함수 호출
}
...
//Tree 에서 Update를 하는 함수 
template<class T>
void FindUpdate(Node<T>* root, T& GetItem,T& SetItem ,bool& Update)
{
	if (root == NULL)						// root가 NULL인 경우 found는 false 
		Update = false;
	else if (GetItem < root->data)				// 찾고자 하는 아이템값이 root값보다 작을 때 
		FindUpdate(root->left, GetItem,SetItem, Update);	// 왼쪽 노드로 가서 retrieve 함수 호출
	else if (GetItem > root->data)				// 찾고자 하는 아이템값이 root값보다 클 때
		FindUpdate(root->right, GetItem,SetItem, Update);	// 오른쪽 노드로 가서 retrieve 함수 호출
	else
	{
	// 찾고자 하는 값과 일치할 때
	//item값에 저장되어있는 값을 data에 넣음
	GetItem = root->data;
	root->data = SetItem;
	Update = true;						// found값을 true로 해서 찾는 과정을 멈춤
	}
}
